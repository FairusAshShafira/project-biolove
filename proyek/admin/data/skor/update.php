    ...
    <div id="wrapper">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
				...
				$id_skor = $_GET['id_skor']; //mengambil id user yang ingin diubah

				//menampilkan barang berdasarkan id
				$data = mysqli_query($koneksi, "SELECT * FROM tbl_skor WHERE id_skor= '$id_skor'");
				...
				?>
                    ...
                    <form method="post" action="">
                        ...
                        <input type="hidden" name="id_skor" value="<?= $row['id_skor']; ?>">
                                ...
                                <select name="username" class="form-control form-control-sm" value="<?= $row['semester']; ?>">
                                    <option value="">Pilih username</option>
                                    <?php
                                    include '../../koneksi.php';

                                    $query = "SELECT DISTINCT username FROM tbl_user WHERE level='user'";
                                    $result = $koneksi->query($query);

                                    if ($result->num_rows > 0) {
                                        while ($rowuser = $result->fetch_assoc()) {
                                            $selected = ($rowuser['username'] == $row['username']) ? 'selected' : '';
                                            echo "<option value='" . $rowuser['username'] . "' $selected>" . $rowuser['username'] . "</option>";
                                        }
                                    }
                                    ?>
                                </select>
                            <br>
                                <label for="" class="form-label">Skor :</label>
                                <input name="score" type="text" class="form-control form-control-sm" id="" placeholder="" value="<?= $row['score']; ?>">
                            <br>
                                &nbsp;
                    ...
                    if (isset($_POST['submit'])) {
                        ...
                        mysqli_query($koneksi, "UPDATE tbl_skor SET username='$username', score='$score' WHERE id_skor='$id_skor'") or die(mysqli_error($koneksi));

                        //redirect ke halaman index.php
                        echo "<script>alert('data berhasil diupdate.');window.location='tables_skor.php';</script>";
